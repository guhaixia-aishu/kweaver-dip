...
func (u *usermgntSvc) GetAccessorIDsByUserID(userID string) (accessorIDs []string, err error) {
	...
	respParam, err := u.httpClient.Get(context.Background(), target, nil)
	if err != nil {
...
func (u *usermgntSvc) GetDepAllUsers(depID string) (userIDs []string, err error) {
	...
	respParam, err := u.httpClientEx.Get(context.Background(), target, nil)
	if err != nil {
...
func (u *usermgntSvc) GetDepAllUserInfos(depID string) (userInfos []UserInfo, err error) {
	fields := "all_users"
	target := fmt.Sprintf("%s/v1/departments/%s/%s", u.baseURL, depID, fields)
	respParam, err := u.httpClientEx.Get(context.Background(), target, nil)
	if err != nil {
...
func (u *usermgntSvc) GetGroupMembers(groupID string) (userIDs, depIDs []string, err error) {
	...
	_, respParam, err := u.httpClient.Post(context.Background(), target, nil, tmpInfo)
	if err != nil {
...
func (u *usermgntSvc) GetUserInfoByID(userID string) (userInfo UserInfo, err error) {
	target := fmt.Sprintf("%s/v1/users/%s/%s", u.baseURL, userID, "groups")
	resp, err := u.httpClient.Get(context.Background(), target, nil)
	if err != nil {
...
func (u *usermgntSvc) GetAccessorIDsByDepartID(depID string) (accessorIDs []string, err error) {
...
func (u *usermgntSvc) GetUserParentDepartments(userID string) (parentDeps [][]Department, err error) {
	endPoint := ContactStr(u.baseURL, "/v1/users/", userID, "/parent_deps")
	respParam, err := u.httpClient.Get(context.Background(), endPoint, nil)
	if err != nil {
