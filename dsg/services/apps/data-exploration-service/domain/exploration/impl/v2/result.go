...
/*
// saveResult 保存探查结果
func (e *ExplorationDomainImplV2) saveResult(tx *gorm.DB, c context.Context, results []map[string]any, reportCode string, projectCode string, field *exploration.ExploreField) error {
	now := time.Now()
...
func (e *ExplorationDomainImplV2) saveAsyncExploreRecord(tx *gorm.DB, c context.Context, taskId string, reportCode string, fields []*exploration.ExploreField) error {
	now := time.Now()
...
/*
// saveAsyncExploreRecord 保存异步探查项目
func (e *ExplorationDomainImplV2) saveAsyncExploreDataRecord(tx *gorm.DB, c context.Context, taskIds []string, sqlMap map[string]string, statisticsSql, mergeSql string, groupMap map[string]string, reportCode string, exploreReq *exploration.DataExploreReq) error {
	now := time.Now()
	items := make([]*model.ReportItem, 0)
	i := 0
	if statisticsSql != "" {
...
*/
// getCache 获取缓存结果
func getCache(exploreReq *exploration.DataExploreReq, e *ExplorationDomainImplV2, c *gin.Context, key string) (*exploration.DataExploreResp, error) {
...
// setCache 设置缓存
func setCache(exploreReq *exploration.DataExploreReq, resp *exploration.DataExploreResp, err error, e *ExplorationDomainImplV2, c *gin.Context, key string) error {
	if exploreReq.Cache == 1 {
...
func (e *ExplorationDomainImplV2) ResultToItem(item *model.ReportItem, result []map[string]any) {
	var res string
	if *item.Project == constant.Quantile {
		...
		*item.Project == constant.Regexp || *item.Project == constant.Unique ||
		...
		res = string(resultBytes)
	} else if *item.Project == constant.NotNULL {
		if result[0][*item.Column] != nil {
			res = fmt.Sprintf("%v", result[0][*item.Column])
		} else {
			res = ""
		}
	} else {
...
func (e *ExplorationDomainImplV2) ExplorationResultUpdate(ctx context.Context, result *exploration.DataASyncExploreResultMsg) (err error) {
...
func (e *ExplorationDomainImplV2) ExplorationResultHandler(ctx context.Context, msg []byte) (err error) {
	defer func(err *error) {
		if e := recover(); e != nil {
			*err = e.(error)
		}
...
// updateReport 更新报告信息
func (e *ExplorationDomainImplV2) updateReport(tx *gorm.DB, ctx context.Context, result *exploration.DataExploreResultMsg, report *model.Report, now time.Time, reportCode string) (err error) {
...
// updateReport 更新报告信息
func (e *ExplorationDomainImplV2) updateExploreReport(tx *gorm.DB, ctx context.Context, result *exploration.DataASyncExploreResultMsg, report *model.Report, now time.Time, reportItem *model.ReportItem) (err error) {
	if result.Result.Status == fail {
		// 项目异常,直接更新报告状态为失败
		report.Status = util.ValueToPtr(constant.Explore_Status_Fail)
		report.FinishedAt = &now
...
func (e *ExplorationDomainImplV2) ThirdPartyExplorationResultHandler(ctx context.Context, msg []byte) (err error) {
...
func (e *ExplorationDomainImplV2) DeleteExploreReport(ctx context.Context, req *exploration.DeleteDataExploreReportReq) (*exploration.DeleteDataExploreReportResp, error) {
	now := time.Now()
