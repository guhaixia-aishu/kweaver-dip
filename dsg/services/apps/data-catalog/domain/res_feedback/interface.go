...
type UseCase interface {
	...
	GetDetail(ctx context.Context, feedbackID uint64, resType string) (*DetailResp, error)
	GetCount(ctx context.Context, uid string) (*CountResp, error)
}

type FeedbackIDPathReq struct {
	FeedbackID models.ModelID `uri:"feedback_id" binding:"TrimSpace,required,VerifyModelID"` // 资源反馈ID
	ResType    string         `uri:"res_type" binding:"TrimSpace"`
}
...
type CreateReq struct {
	ResID        string `json:"res_id" binding:"TrimSpace,required"`                      // 资源ID
	...
	ResType      string `json:"res_type" binding:"TrimSpace"`
}
...
// ResType2Str 将数字类型的ResType转换为字符串类型
func ResType2Str(resType string) string {
	switch resType {
	case "4":
		return S_RES_TYPE_DATA_VIEW
	case "5":
		return S_RES_TYPE_INTERFACE_SVC
	case "6":
		return S_RES_TYPE_INDICATOR
	default:
		// 如果已经是字符串类型，直接返回
		switch resType {
		case S_RES_TYPE_DATA_VIEW, S_RES_TYPE_INTERFACE_SVC, S_RES_TYPE_INDICATOR:
			return resType
		default:
			return ""
		}
...
type ListReq struct {
	...
	Sort            string `form:"sort,default=created_at" binding:"TrimSpace,omitempty,oneof=created_at replied_at res_title" default:"created_at"` // 排序类型，枚举：created_at：按创建时间排序；replied_at：按回复时间排序; catalog_title: 按目录名称排序。默认按创建时间排序
	...
	ResType       string `form:"res_type" binding:"TrimSpace"` // 资源类型 data-view 逻辑视图 interface-svc 接口 服务 indicator 指标
	IndicatorType string `form:"indicator_type" binding:"TrimSpace"`
}
...
func ListReqParam2Map(req *ListReq) map[string]any {
	...
	if len(req.IndicatorType) > 0 {
		rMap["indicator_type"] = req.IndicatorType
	}
	return rMap
}
...
type DetailBasicInfo struct {
	...
	CatalogID     string `json:"res_id"`         // 资源ID
	CatalogCode   string `json:"res_code"`       // 资源code
	CatalogTitle  string `json:"res_title"`      // 资源名称
	...
	FeedbackType  string `json:"feedback_type"`  // 反馈类型（来自数据字典 字典类型：feedback-type）
	...
	InOnline      bool   `json:"is_online"`      // 上线状态
}
