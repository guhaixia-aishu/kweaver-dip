...
import (
	...
	data_resource_catalog_domain "github.com/kweaver-ai/kweaver-dip/dsg/services/apps/data-catalog/domain/data_resource_catalog"
...
type Repo interface {
	Create(tx *gorm.DB, ctx context.Context, m *model.TCatalogFeedback) error
	Update(tx *gorm.DB, ctx context.Context, m *model.TCatalogFeedback, status []int) (bool, error)
	GetByID(tx *gorm.DB, ctx context.Context, ids []uint64) ([]*model.TCatalogFeedback, error)
	GetList(tx *gorm.DB, ctx context.Context, uid string, id uint64, params map[string]any) (int64, []*CatalogFeedbackDetail, error)
	GetCount(tx *gorm.DB, ctx context.Context) (*CountInfo, error)
...
type CatalogFeedbackDetail struct {
	...
	CatalogCode  string     `gorm:"column:catalog_code" json:"catalog_code"`   // 目录CODE
	CatalogTitle string     `gorm:"column:catalog_title" json:"catalog_title"` // 目录名称
