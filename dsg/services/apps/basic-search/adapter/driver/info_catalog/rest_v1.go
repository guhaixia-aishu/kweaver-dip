...
const spanNamePre = "uc InfoCatalogUseCase "
...
func (s RestServiceV1) Search(c *gin.Context) {
	...
	log.WithContext(c.Request.Context()).Infof("info catalog search req: %s", reqJson)
	resp, err := s.uc.Search(c, req)
	if err != nil {
		log.WithContext(c.Request.Context()).Errorf("failed to search info catalog, req: %s, err: %v", reqJson, err)
		s.errResp(c, err)
		return
	}
