...
const spanNamePre = "uc ElecLicenseUseCase "
...
//	@Router			/api/basic-search/v1/info-catalog/search [post]
func (s RestServiceV1) Search(c *gin.Context) {
	...
	log.WithContext(c.Request.Context()).Infof("elec license search req: %s", reqJson)
	resp, err := s.uc.Search(c, req)
	if err != nil {
		log.WithContext(c.Request.Context()).Errorf("failed to search elec license, req: %s, err: %v", reqJson, err)
		s.errResp(c, err)
		return
	}
...
//	@Failure		400				{object}	rest.HttpError					"失败响应参数"
//	@Router			/api/basic-search/v1/info-catalog/statistics [post]
func (s RestServiceV1) Statistics(c *gin.Context) {
	...
	reqJson := string(lo.T2(json.Marshal(req)).A)
	log.Infof("info catalog statistics req: %s", reqJson)
