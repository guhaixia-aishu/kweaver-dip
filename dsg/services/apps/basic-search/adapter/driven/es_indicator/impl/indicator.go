...
import (
	...
	es "github.com/kweaver-ai/kweaver-dip/dsg/services/apps/basic-search/adapter/driven/es_indicator"
...
// 索引名称
const (
	mappingVersionV1 = "af_indicator_idx_v1"
	mappingVersionV2 = "af_indicator_idx_v2"
	mappingVersionV3 = "af_indicator_idx_v3"
)
...
// 索引别名
const (
	indicesAlias = "af_indicator_idx"
	IndicesName  = mappingVersionV3
)
...
func NewObjSearch(esClient *opensearch.SearchClient) (es.ESIndicator, error) {
...
func (e objSearch) Index(ctx context.Context, doc *es.IndicatorDoc) (err error) {
	ctx, span := trace.StartInternalSpan(ctx)
	defer func() { trace.TelemetrySpanEnd(span, err) }()
...
func (e objSearch) Delete(ctx context.Context, id string) (err error) {
	...
	log.WithContext(ctx).Infof("delete indicator doc from es, doc id")
