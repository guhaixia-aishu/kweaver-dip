...
import (
	...
	es "github.com/kweaver-ai/kweaver-dip/dsg/services/apps/basic-search/adapter/driven/es_data_view"
...
type commonSearchParam struct {
	...
	OrgCodes    []string   `json:"orgcodes" binding:"omitempty,dive,uuid" example:"664c3791-297e-44da-bfbb-2f1b82f3b672"` // 所属组织架构ID
...
type DataViewBaseInfo struct {
	...
	OrgName          string             `json:"orgname"` //  所属组织架构ID
...
type SearchResp struct {
	response.PageResult[*DataViewBaseInfo]
	NextFlag []string `json:"next_flag" example:"0.987,abc"` // 获取下一页数据的请求中，需携带本参数，若本参数为空，则数据已全部获取，没有下一页了
}

func NewSearchResp(entries []*DataViewBaseInfo, total int64, next []string) *SearchResp {
	return &SearchResp{
		response.PageResult[*DataViewBaseInfo]{
			Entries:    entries,
			TotalCount: total,
...
func (i *IndexToESReqParam) ToDataViewDoc() *es.DataViewDoc {
	return &es.DataViewDoc{
		DocID: i.DocID,
		BaseObj: es.BaseObj{
			ID:              i.ID,
			Name:            i.Name,
