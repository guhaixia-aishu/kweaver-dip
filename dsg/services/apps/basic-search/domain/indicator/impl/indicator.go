...
type useCase struct {
	esClient es_indicator.ESIndicator
}

func NewUseCase(esClient es_indicator.ESIndicator) domain.UseCase {
...
func (u *useCase) Search(ctx context.Context, req *domain.SearchReqParam) (res *domain.SearchResp, err error) {
	...
	entries := make([]*domain.IndicatorBaseInfo, 0)
	var total int64
	var next []string
	if result != nil {
		for _, item := range result.Items {
			baseInfo := &domain.IndicatorBaseInfo{
				ID:               item.ID,
				Name:             item.Name,
...
func (u *useCase) IndexToES(ctx context.Context, req *domain.IndexToESReqParam) (res *domain.IndexToESRespParam, err error) {
	...
	defer func() { trace.TelemetrySpanEnd(span, err) }()
	if err = u.esClient.Index(ctx, req.ToIndicatorDoc()); err != nil {
		return nil, err
	}
