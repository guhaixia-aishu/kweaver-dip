...
type commonSearchParam struct {
	...
	IsPublish          *bool                 `json:"is_publish,omitempty"`                                                             // 是否发布，如果为 nil 则不根据发布状态过滤搜索结果
	...
	CateInfoR          []es_common.CateInfoR `json:"cate_info,omitempty"`               // 用户筛选的多个类目列表
	Fields             []string              `json:"fields" binding:"omitempty,unique"` // 字段列表。如果非空，关键字仅匹配指定字段
	BusinessObjectIDS  []string              `json:"business_object_ids,omitempty"`
	BusinessProcessIDs []string              `json:"business_process_ids,omitempty"` // 业务流程id, 用户筛选的多个业务流程id
}
...
func (p *commonSearchParam) toESParam() es.BaseSearchParam {
	ret := es.BaseSearchParam{
		...
		CateInfoR:          p.CateInfoR,
		BusinessProcessIDs: p.BusinessProcessIDs,
	}
...
// es_commong中的TimeRange，时间类型是time.Time, 这里是毫秒时间戳
...
type SummaryInfo struct {
	...
	BusinessProcesses []*es_common.BusinessProcess `json:"business_processes"` // 业务流程
	// 信息资源目录 - 业务表
...
func NewSummaryInfo(item *es.SearchResultItem) *SummaryInfo {
	return &SummaryInfo{
		...
		BusinessProcesses: lo.Map(item.BusinessProcesses, func(f *es_common.BusinessProcess, _ int) *es_common.BusinessProcess {
			return &es_common.BusinessProcess{
				BusinessProcessId:     f.BusinessProcessId,     // 业务流程uuid
				BusinessProcessName:   f.BusinessProcessName,   // 业务流程名称
				BusinessProcessPath:   f.BusinessProcessPath,   // 业务流程路径名称
				BusinessProcessPathId: f.BusinessProcessPathId, // 业务流程路径id
			}
		}),
...
func (p *IndexToESReqParam) ToItem() *es.Item {
	ret := es.Item{
		BaseItem: es.BaseItem{
			...
			OnlineStatus:      p.OnlineStatus,
			CateInfo:          p.CateInfo,
			BusinessProcesses: p.BusinessProcesses,
			// 信息资源目录 - 业务表
			BusinessForm: p.BusinessForm,
			// 信息资源目录 - 业务表 - 业务模型
			BusinessModel: p.BusinessModel,
			// 信息资源目录 - 业务表 - 业务模型 - 主干业务 - 部门及其上级部门，为从顶级部门开始
			MainBusinessDepartments: p.MainBusinessDepartments,
			// 信息资源目录 - 业务表 - 业务模型 - 主干业务 - 业务领域
			BusinessDomain: p.BusinessDomain,
			// 信息资源目录 - 数据资源目录
			DataResourceCatalogs: p.DataResourceCatalogs,
			LabelListResp:        p.LabelListResp,
		},
		DocId: p.DocId,
	}
