...
import (
	...
	"github.com/kweaver-ai/kweaver-dip/dsg/services/apps/data-application-gateway/cmd/server/docs"
...
func NewHttpEngine(s *settings.Settings, r IRouter) *gin.Engine {
	...
	app.GET("/api/data-application-gateway/swagger/*any", handleReDoc)
	// 业务绑定路由操作
	err := r.Register(s, app)
	if err != nil {
		panic(err)
	}
