...
在 **找表主流程第 3 步**（`department_duty_query`）调用：基于第 2 步得到的 **部门线索 / 主题域 / 表候选**，查询职责与治理边界，用于回答「谁负责、职责是什么、如何协同」。
...
- **输出**：职责条目、职责动作、适用范围；与第 2 步表候选合并后给出总结。
...
抽取 **部门、职责动作、适用范围**；去重后与第 2 步候选表/视图对照，说明「职责-资产」关系。若为空结果，优先改写 `query`（补充部门名或业务主题）。
